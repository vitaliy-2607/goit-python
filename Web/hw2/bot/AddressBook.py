from abc_method import ABC_Abook
import re
...
class Record(ABC_Abook):
    ...
    def birthday_in_n_days(num_day):
        ...
        d_now = datetime.now()
        day_end = datetime(day=d_now.day, month=d_now.month,
                           year=d_now.year) + num_days
        for k, v in phone_book.data.items():
            try:
                m = datetime(day=v.birthday.value.day,
                             month=v.birthday.value.month, year=d_now.year)
...
def add_birthday(*args):
    ...
    if rec.birthday != None:
        print(
            f'This contact already has birthday date it`s {rec.birthday.value.strftime("%d.%m.%Y")}. Print yes if you want to change it into {args[1]}')
        ask_permision = input('>>>')
...
def add_email(*args):
    ...
    if rec.email != None:
        print(
            f'This contact already has email it`s {rec.email}. Print yes if you want to change it into {args[1]}')
        ask_permision = input('>>>')
        if ask_permision.lower().startswith('yes'):
...
def add_home(*args):
    ...
    if rec.home != None:
        print(
            f'This contact already has home address it`s {rec.home}. Print yes if you want to change it into {args[1]}')
        ask_permision = input('>>>')
        if ask_permision.lower().startswith('yes'):
